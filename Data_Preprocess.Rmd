...
# exposure file
data_path1 = "./BMI.txt"
BMI_raw <- readr::read_delim(data_path1, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
```

```{r}
data_path2 <- "./CAD.txt"
CAD_raw <- readr::read_delim(data_path2, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
```
...
format_data <- function(dat,
  ...
  if(A1_col %in% names(dat)){
    ...
    if(!is.character(dat$A1)){
      message("effect allele column is not character data. Cocrrecting...")
...
```{r}
message("Merge data1 and data2 by SNP ...")
data = merge(BMI, CAD, by="SNP")
```
...
flip.index = which((data$A1.x == data$A2.y & data$A1.y == data$A2.x) |
                       (data$A1.x ==comple(data$A2.y) & data$A1.y == comple(data$A2.x)))

data[,"A1.y"] = data[,"A1.x"]
data[,"A2.y"] = data[,"A2.x"]
data[flip.index ,"Z.y"] = -data[flip.index ,"Z.y"]
```
...
merged_v2  = merge(data, ld, by="SNP")
```
